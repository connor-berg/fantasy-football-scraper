...
class LoadTableResource(Construct):
    ...
    def chunk_requests(self):
        f"""Yield successive {self.MAX_PUT_REQUESTS}-sized chunks from the resource's requests."""
        for i in range(0, len(self.requests), self.MAX_PUT_REQUESTS):
            yield self.requests[i:i + self.MAX_PUT_REQUESTS]
    ...
    def __init__(
        ...
        for i, chunked_request in enumerate(chunked_requests):
            ...
            sdk_call = cr.AwsSdkCall(
                ...
                parameters={
                    "RequestItems": {
                        table.table_name: chunked_request
                    }
                },
            ...
            resource = cr.AwsCustomResource(self, f"LoadTableResource_{i}",
                                            on_create=sdk_call,
                                            policy=policy,
